...
from playlist_maintainer.application_interfaces.exporters.export_to_text_file import (
    export_to_text_file,
)
from playlist_maintainer.application_interfaces.exporters.display_to_terminal import (
    display_playlist_to_terminal,
)
...
class PlaylistService:
    ...
    def _extract_playlist_id(self, identifier: str) -> str:
        ...
        if not re.match(pattern, identifier):
            raise ValueError(
                "A Valid YouTube Playlist identifier contains just alphanumeric characters, with '-' or '_'"
            )

        if len(identifier) < 20:
            print(
                f"WARNING: Identifier '{identifier}' looks too short to be a valid YouTube playlist ID. Atempting to use it as is."
            )
    ...
    def export_playlist_to_file(
        ...
        try:
            ...
            print(
                f"ERROR(Service): Failed to fetch playlist items for ID '{playlist_id}': {e}"
            )
            raise
        ...
        if not re.match(_pattern, filename):
            raise ValueError(
                "A Valid filename for ytpl contains just alphanumeric characters, plus '-', '_' and '.'"
            )

        os.makedirs(output_dir_path, exist_ok=True)
        expanded_output_dir_path = os.path.expanduser(output_dir_path)
        full_output_file_path = os.path.join(
            expanded_output_dir_path, f"{filename}_{output_format.lower()}"
        )

        try:
            # TODO: Properly retrieve playlist title
            if output_format == "pdf":
                export_to_pdf_file(playlist_items, full_output_file_path)
            elif output_format == "txt":
                export_to_text_file(playlist_items, full_output_file_path)
            else:
                raise ValueError(f"Unsupported export format: {output_format}")
        except Exception as e:
            print(
                f"ERROR(Service): Failed to export {output_format} file to '{full_output_file_path}': {e}"
            )
            raise

    def show_playlist_in_terminal(self, playlist_identifier: str, limit: int):
        """
        Displays a playlist items in terminal.
        """
        playlist_id = self._extract_playlist_id(playlist_identifier)
        try:
            playlist_items = self.api_client.get_playlist_items(playlist_id)
        ...
        try:
            ...
            print(
                f"ERROR(Service): Failed to fetch playlist details for ID '{playlist_id}': {e}"
            )
            raise

        display_playlist_to_terminal(playlist_details, playlist_items, limit)
